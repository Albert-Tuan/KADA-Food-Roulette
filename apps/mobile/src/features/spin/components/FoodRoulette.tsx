...
export function FoodRoulette({ candidates, onSpinEnd, disabled = false }: FoodRouletteProps) {
  const rotation = useSharedValue(0);
  const [spinning, setSpinning] = React.useState(false);

  const segmentAngle = candidates.length > 0 ? 360 / candidates.length : 360;

  const handleSpinEnd = useCallback((winner: Restaurant, index: number) => {
    setSpinning(false);
  ...
  const renderSegments = () => {
    ...
    return candidates.map((candidate, index) => {
      ...
      const displayName = candidate.name.length > 12
        ? candidate.name.substring(0, 11) + '…'
        : candidate.name;
