...
function createInstallationId(): Uint8Array {
  const runtime = globalThis as typeof globalThis & {
    crypto?: CryptoProvider;
    expo?: ExpoRuntime;
  };
  const nativeUuid = runtime.expo?.uuidv4?.();
  if (nativeUuid) {
    return Uint8Array.from(nativeUuid, (character) => character.charCodeAt(0));
  }

  if (runtime.crypto?.getRandomValues) {
    return runtime.crypto.getRandomValues(new Uint8Array(32));
  }

  throw new Error('Thiết bị không hỗ trợ nguồn ngẫu nhiên an toàn. Vui lòng cập nhật ứng dụng.');
}
